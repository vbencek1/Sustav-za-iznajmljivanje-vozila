 ...
 * Poziva se kada korisnik želi dobiti informaciju o tome koliko ima slobodnih
 * mjesta za punjenje vozila na izabranoj lokaciji
 *
 * @author vbencek
 */
public class PregledMjesta implements Aktivnost {
    ...
    /**
     * Metoda koja prikazuje korisniku koliko ima slobodnih mjesta za punjenje
     * vozila na lokaciji.
     *
    ...
    public void izvrsi(String operacija) {
        ...
        if (provjeri(operacija) && provjeriIspravnoVrijeme(vrijeme)) {
            ...
            int brojMjesta = ListaLokacijeKapaciteti.getInstanca().vratiBrojRaspolozivihMjesta(lokacija, vozilo);
            if (brojMjesta < 0) {
                System.out.println("Pregled mjesta: Lokacija sa navedenim vozilom ne postoji");
            } else {
                int ukupno = ListaLokacijeKapaciteti.getInstanca().UkupniBrojMjesta(lokacija, vozilo);
                System.out.println("Pregled mjesta: Na lokaciji ima " + brojMjesta + " slobodnih mjesta. Ukupno ima " + ukupno + " raspolozivih mjesta");
            }
        }
    }
